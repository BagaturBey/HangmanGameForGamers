  ...
  for (let i = 65; i < 91; i++) {
    ...
    button.addEventListener("click", () => {
      ...
      if (charArray.includes(button.innerText)) {
        charArray.forEach((char, index) => {
          ...
          if (char === button.innerText) {
            ...
            if (winCount == charArray.length) {
              resultText.innerHTML = `<h2 class='win-msg'>You Win!!</h2><p>The chosen game was <span>${chosenWord}</span></p>`;
              //block all buttons
              blocker();
            }
          }
        ...
        if (count == 6) {
          resultText.innerHTML = `<h2 class='lose-msg'>You Lose!!</h2><p>The chosen game was <span>${chosenWord}</span></p>`;
          blocker();
        }
      }
